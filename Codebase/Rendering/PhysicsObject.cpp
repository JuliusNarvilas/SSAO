...
{
	m_ColShape = new btSphereShape(static_cast<btScalar>(radius));
	return true;
}
...
bool PhysicsObject::CreateCollisionShape(double radius, double height, CollisionShape shape)
{
	if (shape == CONE)
	{
		m_ColShape = new btConeShape(static_cast<btScalar>(radius), static_cast<btScalar>(height));
		return true;
	}
	else if (shape == CAPSULE)
	{
		m_ColShape = new btCapsuleShape(static_cast<btScalar>(radius), static_cast<btScalar>(height));
		return true;
	}
	return false;
}
...
bool PhysicsObject::CreateCollisionShape(double distance, const Vec3Physics& normal, bool normalised)
{
	...
	m_ColShape = new btStaticPlaneShape(btVector3(normal.x, normal.y, normal.z), static_cast<btScalar>(distance));
	return true;
}
