...
PyramidCollisionShape::PyramidCollisionShape(const Vec3Physics& scale) : CollisionShape(CollisionShapeType::CollisionCuboid) {
	...
	m_Normals[0] = (m_Points[0] - m_Points[3]).Cross(m_Points[4] - m_Points[3]).Normalize();
	m_Normals[1] = (m_Points[1] - m_Points[0]).Cross(m_Points[4] - m_Points[0]).Normalize();
	m_Normals[2] = (m_Points[2] - m_Points[1]).Cross(m_Points[4] - m_Points[1]).Normalize();
	m_Normals[3] = (m_Points[3] - m_Points[2]).Cross(m_Points[4] - m_Points[2]).Normalize();
...
void PyramidCollisionShape::ConstructPyramidHull() {
	...
	m_PyramidHull.AddFace((v0 - v3).Cross(v4 - v3).Normalize(), 3, face1);
	m_PyramidHull.AddFace((v1 - v0).Cross(v4 - v0).Normalize(), 3, face2);
	m_PyramidHull.AddFace((v2 - v1).Cross(v4 - v1).Normalize(), 3, face3);
	m_PyramidHull.AddFace((v3 - v2).Cross(v4 - v2).Normalize(), 3, face4);
